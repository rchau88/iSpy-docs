...
The iSpy's dashboard relies on four Docker containers to run its genome assembly and curation: **IRMA**, **DAIS-Ribosome**, **spyne**, and **iSpy**. Presently, the IRMA and DAIS-Ribosome docker images are stored in **AWS’s EC**, and the spyne and iSpy docker images are stored in **Quay.io's repository**. These images can be pulled down and built into runnable containers with the ```docker pull``` and ```docker run``` commands.
...
### (4) Extract test data for running iSpy and spyne
...
### (5) CD to `iSpy` folder where `docker-compose-quay-dev.yml` is stored and edit the yaml file to link the data inputs and code base to appropriate containers
